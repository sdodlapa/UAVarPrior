...
class IntervalsSampler(OnlineSampler):
    ...
    def _retrieve(self, chrom, position):
        ...
        retrieved_targets = self.target.getFeatureData(
        ...
        if self.mode in self._save_datasets:
            ...
            if len(self._save_datasets[self.mode]) > 200000:
                self.saveDatasetToFile(self.mode)
    ...
    def _update_randcache(self, mode=None):
        ...
        mode : str or None, optional
            ...
            used for. See `fugep.samplers.IntervalsSampler.modes` for
            more information.
    ...
    def sample(self, batchSize=1, mode=None, autoReset = True):
        """
        ...
        sequences, targets : tuple(numpy.ndarray, numpy.ndarray)
            ...
            `batchSize`, :math:`L` is the sequence length, and
        ...
        sequences = np.zeros((batchSize, self.sequence_length, 4))
        targets = np.zeros((batchSize, self.n_features))
        n_samples_drawn = 0
        while n_samples_drawn < batchSize:
