...
from ...evaluate.metrics import f1Neg
...
class StandardSGDTrainer(SGDTrainer):
    ...
    def __init__(self,
                 ...
                 newClassifier = None,
                 freezeStem = None,
                 metrics=dict(roc_auc=roc_auc_score, precision=average_precision_score,
                              f1_pos=f1_score, f1_neg=f1Neg, accuracy=accuracy_score, recall=recall_score,
                              # confusion_TnFpFnTp=confusion_matrix
                              ), # Used in combined validation and evaluation
                 useScheduler=True,
                 deterministic=False,
    ...
    def trainAndValidate(self):
        ...
        """
        logger.info("Pytorch model summary: {}".format(torchinfo.summary(self.model._model, input_size=(self.batchSize, 4, self.sampler.getSequenceLength()))))
    ...
    def _train(self):
        ...
        sumOfLoss, nEffTerms = self.model.fit(batchData)
        # track the loss
    ...
    def _validate(self, step):
        ...
        to_log = [str(loss)]
        # if score is None:
        #     logger.info("Its going to fail XXXXXXXXXXXXXxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
