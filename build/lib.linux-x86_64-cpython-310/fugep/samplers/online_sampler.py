...
from ..data import GenomicFeatures
...
class OnlineSampler(Sampler, metaclass=ABCMeta):
    ...
    def getDataAndTargets(self, batchSize, nSamps=None, mode=None):
        ...
        nSamps : int or None, optional
            ...
            If `nSamps` is None and the mode is `validate`, will
            set `nSamps` to 32000; if the mode is `test`, will set
            `nSamps` to 640000 if it is None. If the mode is `train`
            you must have specified a value for `nSamps`.
        mode : str, optional
            Default is None. The mode to run the sampler in when
            fetching the samples. See
            `fugep.samplers.IntervalsSampler.modes` for more
        ...
        targets_mat = []
        if nSamps is None and mode == "validate":
            nSamps = 32000
        elif nSamps is None and mode == "test":
            nSamps = 640000

        n_batches = int(nSamps / batchSize)
        for _ in range(n_batches):
            batchData = self.sample(batchSize)
            sequences_and_targets.append(batchData)
            targets_mat.append(batchData['targets'])
        targets_mat = np.vstack(targets_mat)
        if mode in self._save_datasets:
            self.saveDatasetToFile(mode, close_filehandle=True)
        return sequences_and_targets, targets_mat

    def get_dataset_in_batches(self, mode, batchSize, nSamps=None):
        """
        ...
        mode : {'test', 'validate'}
            ...
            See `fugep.samplers.IntervalsSampler.modes` for more
            information.
        batchSize : int
            The size of the batches to divide the data into.
        nSamps : int or None, optional
        ...
        tuple(list(tuple(numpy.ndarray, numpy.ndarray)), numpy.ndarray)
            Tuple containing the list of sequence-target pairs, as well
            as a single matrix with all targets in the same order.
            The list is length :math:`S`, where :math:`S =` `nSamps`.
            Note that `sequences_and_targets`'s sequence elements are of
            the shape :math:`B \\times L \\times N` and its target
            elements are of the shape :math:`B \\times F`, where
            :math:`B` is `batchSize`, :math:`L` is the sequence length,
        ...
        return self.getDataAndTargets(
            batchSize, nSamps=nSamps, mode=mode)
    ...
    def getValidationSet(self, batchSize, nSamps=None):
        ...
        return self.get_dataset_in_batches(
            "validate", batchSize, nSamps=nSamps)
    ...
    def getTestSet(self, batchSize, nSamps=None):
        ...
        if "test" not in self.modes:
            raise ValueError("No test partition of the data was specified "
                             ...
                             "`getTestSet`.")
        return self.get_dataset_in_batches("test", batchSize, nSamps)
