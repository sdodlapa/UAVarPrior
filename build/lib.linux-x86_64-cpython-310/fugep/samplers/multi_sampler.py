...
from .file_samplers.file_sampler import FileSampler
...
class MultiSampler(Sampler):
    ...
    testSampler : None or FileSampler or DataLoader, optional
    ...
    def __init__(self,
        ...
        self._samplers = {
            "train": trainSampler if (isinstance(trainSampler, Sampler) 
                                       or isinstance(trainSampler, FileSampler)) \
                     else None,
            "validate": validateSampler if (isinstance(validateSampler, Sampler)
                                             or isinstance(validateSampler, FileSampler)) \
                        else None
        }

        self._dataloaders = {
            "train": trainSampler if isinstance(trainSampler, DataLoader) \
                     else None,
            "validate": validateSampler if isinstance(validateSampler, DataLoader) \
                        else None
        }
        ...
        self._indexToFeature = {i: f for (i, f) in enumerate(features)}

        if testSampler is not None:
            self.modes.append("test")
            self._samplers["test"] = \
                testSampler if (isinstance(testSampler, Sampler) 
                                 or isinstance(testSampler, FileSampler)) else None
            self._dataloaders["test"] = \
                testSampler if isinstance(testSampler, DataLoader) else None
    ...
    def _setBatchSize(self, batch_size, mode=None):
        """
    ...
    def sample(self, batch_size=1, mode=None):
        ...
        if self._samplers[mode]:
            ...
            self._setBatchSize(batch_size, mode=mode)
            try:
    ...
    def getDataAndTargets(self, batch_size, n_samples=None, mode=None):
        """
        ...
        mode = mode if mode else self.mode
        if self._samplers[mode]:
            return self._samplers[mode].getDataAndTargets(
    ...
    def getValidationSet(self, batch_size, n_samples=None):
        """
    ...
    def getTestSet(self, batch_size, n_samples=None):
        """
