...
class Genome(Sequence):
    ...
    def init(func):
        ...
        def dfunc(self, *args, **kwargs):
            if not self._initialized: #by Sanjeev 08/26/2022
                self._unpicklable_init()
    ...
    @classmethod
    def getComplementEncoding(cls, seqEnc):
        '''
        Get the encoding of the complement of the sequence represented by
        the input seqEnc
        '''
        complementIndices = [
            cls.BASE_TO_INDEX[cls.COMPLEMENTARY_BASE_DICT[b]] for b in cls.BASES_ARR]
        return seqEnc[:, complementIndices]
