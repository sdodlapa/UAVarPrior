...
class DeepSVSDTrainer(object):
    ...
    def __init__(self,
                 ...
                 output_dir,
                 nu,  # hyperparameter
                 objective,  # 'ocl' or 'soft-boundary'?
                 batch_size,
                 max_steps,
                 report_stats_every_n_steps,
        ...
        self.objective = objective  # objective should be soft-boundary or one-class classification
        assert (0 < nu) & (nu <= 1), "For hyperparameter nu, it must hold: 0 < nu <= 1."
        self.nu = nu
        # deep SVDD parameter
        self.R = 0.0
        # self.c = torch.tensor(c) if c is not None else None
        self.C = None
        ...
        self.warm_up_n_steps = 10  # number of training steps for soft-boundary Deep SVDD before radius R gets updated
    ...
    def train(self, step):
        ...
        if self.C is None:
            ...
            self.C = self.init_center_c(inputs, self.model, 0.1)
        ...
        out_put, predict = self.model(inputs.transpose(1, 2))
        dist = torch.sum((out_put - self.C)**2, dim=1)
        loss = self.criterion(dist, self.R, self.objective, self.nu)
        # dist, loss = self.criterion(reshape_out, self.c)

        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()

        #Update hypersphere radius R on a batch distances
        if (self.objective == 'soft-boundary') and (step >= self.warm_up_n_steps):
            self.R.data = torch.tensor(get_radius(dist, self.nu), device=self.device)

        return loss.item()

    def init_center_c(self, inputs, model, eps=0.1): #calculate center c
        ...
        with torch.no_grad():
            for data in inputs:
                ...
                predictions = self.model(data.transpose(1, 2))
                n_samples += predictions.shape[0]
                c += torch.sum(predictions, dim=0)
    ...
    def _evaluate_on_data(self, data_in_batches):
        ...
        for (inputs, targets) in data_in_batches:
            ...
            with torch.no_grad():
                ...
                loss = self.criterion(dist, R, objective, nu)
    ...
    def validate(self):
        ...
        logger.info("{0} items < 0.50, {1} items ≥ 0.5 in all_predictions".format(
            np.sum(all_predictions < 0.5), np.sum(all_predictions >= 0.5)))
        logger.info("{0} items < 0.50, {1} items ≥ 0.5 in all_validation_targets".format(
            len(self._all_validation_targets) - np.sum(self._all_validation_targets),
            np.sum(self._all_validation_targets)))
