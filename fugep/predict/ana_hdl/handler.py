...
def write_to_mult_tsv_files(data_across_features, info_cols, output_filepath):
    # file_stem = output_filepath.split('-')[0]
    # tail = output_filepath.split('-')[1]
    # tail = tail.split('.')[0]
    head, tail = os.path.split(output_filepath)
    head = os.path.join(head, tail.split('-')[0])
    tail = tail.split('-')[1]
    tail = tail.split('.')[0]
    if len(data_across_features) > 0:
        for i in range(data_across_features[0].shape[0]):
            out_path = head+'-'+tail+'_'+str(i)+'.tsv'

            with open(out_path, 'a') as output_handle:
                temp = [feature[i, :, :] for feature in data_across_features]
                for info_batch, preds_batch in zip(info_cols, temp):
                    for info, preds in zip(info_batch, preds_batch):
                        preds_str = '\t'.join(
                            probabilities_to_string(list(preds)))
                        info_str = '\t'.join([str(i) for i in info])
...
def write_to_hdf5_file(data_across_features,
    ...
    with h5py.File(hdf5_filepath, 'a') as hdf5_handle:
        ...
        # data.resize(data_len+start_index, 0)

        for data_batch in data_across_features:
            if data.shape[0] >= start_index + data_batch.shape[0]:
                data[start_index :(start_index + data_batch.shape[0])] = data_batch
                start_index = start_index + data_batch.shape[0]
            else:
                data.resize(start_index + data_batch.shape[0], 0)
                data[start_index:(start_index + data_batch.shape[0])] = data_batch
                # print(f'data shape: {data.shape}, data_batch shape: {data_batch.shape}')
                start_index = start_index + data_batch.shape[0]
...
def probabilities_to_string(probabilities):
    ...
    """
    try:
        text_list = ["{:.2e}".format(p) for p in probabilities]
    except:
        text_list = ["{:.2e}".format(p[0]) for p in probabilities]
...
class PredictionsHandler(metaclass=ABCMeta):
    ...
    def _create_mult_write_handler(self, handler_filename, num_files):
        """
        Initialize handlers for writing outputs to file.
        ...
        if self._output_format == "tsv":
            if num_files > 1:
                for n in range(num_files):
                    temp_path = scores_filepath+'_'+str(n)
                    self._output_filepath = "{0}.tsv".format(temp_path)
                    with open(self._output_filepath, 'w+') as output_handle:
                        column_names = self._columns_for_ids + self._features
                        output_handle.write("{0}\n".format(
                            '\t'.join(column_names)))
    ...
    def _reached_mem_limit_temp(self):
        mem_used = (self._allResults[0].nbytes * len(self._allResults) * 2 +
                    getsizeof(self._samples[0]) * len(self._samples))
        return mem_used / 10**6 >= self._write_mem_limit
